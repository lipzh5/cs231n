...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param, normalization, dropout, do_param):
  ...
  # normalization
  if normalization:
    norm_forward_func = batchnorm_forward if normalization == 'batchnorm' else layernorm_forward
    out, bn_cache = norm_forward_func(out, gamma, beta, bn_param)
  # relu
...
def affine_norm_relu_backward(dout, cache, normalization, dropout):
  ...
  if normalization:
    norm_backward_func = batchnorm_backward_alt if normalization == 'batchnorm' else layernorm_backward
    dout, dgamma, dbeta = norm_backward_func(dout, bn_cache)
  # affine layer
