...
def affine_forward(x, w, b):
    ...
    x2 = x.reshape(x.shape[0], -1)  # (N, D)
    out = np.dot(x2, w) + b   # (N, M)
...
def affine_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    num_train = x.shape[0]
    x2 = x.reshape(num_train, -1)
    dw = np.dot(x2.T, dout) # (D, N) *（N, M） ==> (D, M)
    dx = np.dot(dout, w.T)  # (N,M) * (M, D)  ==> (N, D)
    db = np.dot(dout.T, np.ones(num_train))           # (M, N) * (N, ) ==> (M, 1)
    dx = dx.reshape(x.shape) # reshape to the shape as input x
...
def affine_norm_relu_backward(dout, cache, normalization, dropout):
  ...
  if normalization == 'batchnorm':
    ...
    douot, dgamma, dbeta = layernorm_backward(dout, bn_cache)
  # affine layer
...
def svm_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = x.shape[0]  # 
    margins = np.maximum(0, x - np.choose(y, x.T).reshape(-1, 1) + 1)
    # correct_class_scores = x[np.arange(N), y]
    # margins = np.maximum(0, x - correct_class_scores.reshape(-1, 1) + 1)
    margins[np.arange(N), y] = 0
    loss = np.sum(margins)/N
    sum_per_row = np.sum(margins>0, axis=1)
    dx = np.zeros_like(margins)
    dx[margins>0] = 1
   
    dx[np.arange(N), y] -= sum_per_row
    dx /= N
...
def softmax_loss(x, y):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    probs = np.exp(x-np.max(x, axis=1, keepdims=True)) # for numerically stable, values stay the same
