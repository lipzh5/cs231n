...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # project image features into the same dimension as the test embeddings
        # shape: (N, D) -> (N, W) -> (N, 1, W)
        projected_features = self.visual_projection(features).unsqueeze(1) 
        # an additive mask for masking the future (one direction)
        # shape: (T, T)
        tgt_mask = torch.tril(torch.ones(T,T,
                  device=caption_embeddings.device, dtype=caption_embeddings.dtype))
        # apply the Transformer decoder to the caption, allowing it to also attend image features
        features = self.transformer(tgt=caption_embeddings, 
                    memory=projected_features, tgt_mask=tgt_mask)
