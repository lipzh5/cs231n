...
def affine_backward(dout, cache):
    ...
    """
    x, w, b = cache
    dx = dout.dot(w.T).reshape(x.shape) # (N,D)<-(N,M)(M,D)|dout.dot(w.T)
    dw = x.reshape(x.shape[0], -1).T.dot(dout)
    db = np.sum(dout, axis=0) # sum per-column
    return dx, dw, db
...
def rnn_step_backward(dnext_h, cache):
    ...
    # d(tanh) = 1- tanh*tanh
    dh_raw = (1-next_h*next_h) * dnext_h  # (4,6)
    dWx = x.T.dot(dh_raw)
    dx = dh_raw.dot(Wx.T) 
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape  
    N, H = h0.shape
    cache = {}   # cache for all the time steps
    h = np.empty((N,T,H))
    prev_h = h0 
    for t in range(T):
      # xin = x[:, t, :].reshape(N, D)
      # print('xin : ', xin)
      # print('shape of xin: ', xin.shape)
      prev_h, cache_ = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
      # print('shape of next h: ', next_h.shape)
      # print('shape of h: ', h.shape)
      h[:,t,:] = prev_h 
      # prev_h = next_h     # update hidden state for next timestamp
      cache[t] = cache_
      # cache.append(cache_)    
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
      # print('time step t: ', t)
      # dprev_h += dh[:,t,:].reshape(N, H) 
      # dxt, dprev_h, dWxtmp, dWhtmp, dbtmp = rnn_step_backward(dprev_h, cache[t]) 
      dnext_h = dh[:,t,:] + dprev_h 
      # print('$$ shape dht:  ', dh[:,t,:].shape)
      # print('shape of dprev_h ', dprev_h.shape)
      dxt, dprev_h, dWxtmp, dWhtmp, dbtmp = rnn_step_backward(dnext_h, cache[t])
      # print('shape of dprev_h: ', dprev_h.shape)  # (2,5)
      dx[:,t,:] = dxt
      dWx += dWxtmp
      dWh += dWhtmp
      db += dbtmp
      # print('after calc dprev h: ', dprev_h.shape)  # (2,5)
    dh0 = dprev_h
...
def word_embedding_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, W = cache
    dW = np.zeros_like(W)   # (5,6)
    # ref: https://numpy.org/doc/stable/reference/generated/numpy.ufunc.at.html
    # add items (indexed by x) in dW (the first dimension of dW) , add to dout, and stores results in dW 
    # e.g., a=np.array([[1,2,3,4],[5,6,7,8]]), b=np.array([1,2,3,4]), x=np.array([1])
    # np.add.at(a, x, b) ==> a: np.array([[1,2,3,4],[6,8,11,11]])
    np.add.at(dW, x, dout) 
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    H = prev_h.shape[1]
    #print('type of Wx', type(Wx))
    stack_all = x @ Wx + prev_h @ Wh + b
    gi = sigmoid(stack_all[:, 0:H])
    gf = sigmoid(stack_all[:, H:2*H])
    go = sigmoid(stack_all[:, 2*H:3*H])
    gg = np.tanh(stack_all[:, 3*H:])
    next_c = gf * prev_c + gi * gg
    next_h = go * np.tanh(next_c)
    cache = gi, gf, go, gg, x, prev_h, prev_c, next_c, Wx, Wh 
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    gi, gf, go, gg, x, prev_h, prev_c, next_c, Wx, Wh = cache
    dnext_c_plus = dnext_c + go * (1-np.power(np.tanh(next_c), 2))*dnext_h
    da = np.concatenate(
      (dnext_c_plus * gg * gi * (1-gi),
      dnext_c_plus * prev_c * gf * (1-gf),
      dnext_h * np.tanh(next_c) * go * (1-go),
      dnext_c_plus * gi * (1-np.power(gg, 2))), 
      axis=1)
    
    dx = da @ Wx.T
    dprev_h = da @ Wh.T
    dprev_c = (dnext_c_plus) * gf
    dWx = x.T @ da
    dWh = prev_h.T @ da
    db = np.sum(da, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N, T, H))
    for t in range(T):
      prev_h, prev_c, grad_cache[t] = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)
      h[:, t,:] = prev_h
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):  
      dx_m, dprev_h, dprev_c, dWx_m, dWh_m, db_m = lstm_step_backward(dprev_h+dh[:,t,:], dprev_c,grad_cache[t])
      dx[:, t, :] = dx_m
      dWx += dWx_m
      dWh += dWh_m
      db += db_m
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    N, T, V = x.shape  #  scores over all vocabulary elements at all timesteps

    x_flat = x.reshape(N * T, V)
    y_flat = y.reshape(N * T)
    mask_flat = mask.reshape(N * T)
    # print('shape of x: ', x.shape)  # (7,8,9) ==> (N,T,V)
    # print('shape of mask', mask.shape) # (7,8)
    # print('shape of mask flat: ', mask_flat.shape) # (56, )

    probs = np.exp(x_flat - np.max(x_flat, axis=1, keepdims=True))
    probs /= np.sum(probs, axis=1, keepdims=True)
    # print('shape of probs: ', probs.shape)  # (56, 9) ==>(N*T,V)
