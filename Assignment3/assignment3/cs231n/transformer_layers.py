...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # PE(i, j) = sin(i*10000^(-j/d))  if j is even else cos(i*10000^(-(j-1)/d))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        - output: Tensor of shape (N, S, E) giving the weighted combination of
          ...
          and query.
        """
        N, S, E = query.shape   # S: source sequence len, T: target sequence len
        N, T, E = value.shape
        # Create a placeholder, to be overwritten by your code below.
        output = torch.empty((N, S, E))
        # print('inspect torch empty: ', output.shape, output.size())
        ############################################################################
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        H = self.n_head
        splitted_queries = self.query(query).reshape(N, S, H, E//H).transpose(1,2)  # (N, H, S, E//H)
        splitted_keys = self.key(key).reshape(N, T, H, E//H).transpose(1, 2)
        splitted_values = self.value(value).reshape(N, T, H, E//H).transpose(1,2)
        ratio = torch.matmul(splitted_queries, splitted_keys.transpose(2,3)) /math.sqrt(E/H)
        if attn_mask is not None:
          ratio = ratio.masked_fill(attn_mask == 0, -float('inf'))
        attn_probs = F.softmax(ratio, -1)  # attention probs
        output = self.attn_drop(attn_probs)  # dropout some probs
        output = torch.matmul(output, splitted_values)
